...
class Reporte_economico_excel extends Spreadsheet
{
    ...
    public function reporte_economico_curso($data_course, $data_students, $total_inscrito, $total_preinscrito, $total_agrupacion_incritos, $total_agrupacion_preinscritos)
    {
        ...
        $this->getActiveSheet()->getColumnDimension('A')->setWidth(10);
        $this->getActiveSheet()->getColumnDimension('B')->setWidth(19);
        ...
        if ($data_students != null) {
            ...
            // Print data agrupacion por tipo de pago
            $cn = 4;
            $contador_inscritos = $contador;
            $fila = ["A", "B", "C"];
            $boldArray = array('font' => array('bold' => true, 'name' => 'Arial', 'size' => 9));
            $this->getActiveSheet()->getStyle('A' . intval($contador + $cn) . ':C' . intval($contador + $cn))->applyFromArray($boldArray)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            foreach (["CANTIDAD", "TIPO PAGO", "TOTAL PAGO INSCRITOS"] as $key => $value) {
                $this->getActiveSheet()->setCellValue($fila[$key] . intval($contador + $cn), $value);
            }

            $rango1 = "A" . intval($contador + $cn) . ":C" . intval($contador + $cn);
            $styleArray1 = [
                'borders' => ['allBorders' => [
            ...
            $this->getActiveSheet()->getStyle($rango1)->applyFromArray($styleArray1);

            $cn = 5;
            foreach ($total_agrupacion_incritos as $key => $value) {
                $this->getActiveSheet()->setCellValue("A" . intval($contador + $cn), $value->cantidad);
                $this->getActiveSheet()->setCellValue("B" . intval($contador + $cn), $value->tipo_pago);
                $this->getActiveSheet()->setCellValue("C" . intval($contador + $cn), $value->monto_pago);
                $rango1 = "A" . intval($contador + $cn) . ":C" . intval($contador + $cn);
                $styleArray1 = [
                    'borders' => ['allBorders' => [
                        'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK,
            ...
            // Print data agrupacion por tipo de pago PREINSCRITOS
            $cn = 11;
            $fila = ["A", "B", "C"];
            $boldArray = array('font' => array('bold' => true, 'name' => 'Arial', 'size' => 9));
            $this->getActiveSheet()->getStyle('A' . intval($contador + $cn) . ':C' . intval($contador + $cn))->applyFromArray($boldArray)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            foreach (["CANTIDAD", "TIPO PAGO", "TOTAL PAGO PREINSCRITOS"] as $key => $value) {
                $this->getActiveSheet()->setCellValue($fila[$key] . intval($contador + $cn), $value);
                $rango1 = "A" . intval($contador + $cn) . ":C" . intval($contador + $cn);
                $styleArray1 = [
                    'borders' => ['allBorders' => [
            ...
            $cn = 12;
            foreach ($total_agrupacion_preinscritos as $key => $value) {
                $this->getActiveSheet()->setCellValue("A" . intval($contador + $cn), $value->cantidad);
                $this->getActiveSheet()->setCellValue("B" . intval($contador + $cn), $value->tipo_pago);
                $this->getActiveSheet()->setCellValue("C" . intval($contador + $cn), $value->monto_pago);
                $rango1 = "A" . intval($contador + $cn) . ":C" . intval($contador + $cn);
