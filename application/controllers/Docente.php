...
class Docente extends PSG_Controller
{
    ...
    {
        $this->templater->view('docente/index', $this->data);
    }
    ...
    public function search_course()
    {
        if ($this->input->get('searchTerm') == null) {
            ...
            $courses = $this->docente_model->list_courses(strtoupper($this->input->get('searchTerm')));
            $json = [];
            foreach ($courses as $key => $course) {
                $json[] = ['id' => $course['id'], 'text' => $course['fullname'] . ' - ' . $course['shortname']];
            }
        }
    ...
    public function search_user()
    {
        if ($this->input->get('searchTerm') == null) {
            ...
            $users = $this->docente_model->list_user(strtoupper($this->input->get('searchTerm')));
            $json = [];
            foreach ($users as $key => $user) {
                $json[] = ['id' => $user['id'], 'text' => $user['firstname'] . '  ' . $user['lastname'] . ' - ' . $user['email']];
            }
        }
        echo json_encode($json);
    }
    ...
    public function guardar_docente()
    {
        ...
        $response = $this->sql_ssl->listar_tabla(
            'inscripcion_curso',
            [
                "id_user_moodle" => $id_user,
                "id_course_moodle" => $id_course
            ]
        );
        if (count($response) == 0) {
            $response = $this->sql_ssl->insertar_tabla(
                ...
                [
                    ...
                    "monto_pago" => 0,
                    "tipo_participacion" => "EXPOSITOR",
                    "certificacion_unica" => 'CURSO',
                    "id_usuario" => 1,
                    "estado_inscripcion_curso" => 'REGISTRADO'
                ]
            );
            ...
            if (is_numeric($response)) {
                $this->output->set_content_type('application/json')->set_output(json_encode(
                    [
                        'exito' => "Docente registrado correctamente"
                    ]
                ));
            } else {
                $this->output->set_content_type('application/json')->set_output(json_encode(
            ...
            $this->output->set_content_type('application/json')->set_output(json_encode(
                [
                    'error' => 'El docente ya esta registrado'
                ]
            ));
        }
    }
